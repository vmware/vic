...
Install Harbor To Test Server
    ...
    ${out}=  Run  wget https://github.com/vmware/harbor/releases/download/0.4.5/harbor_0.4.5_beta.ova
    ${out}=  Run  ovftool harbor_0.4.5_beta.ova harbor_0.4.5_beta.ovf
    ${out}=  Run  ovftool --datastore=${datastore} --name=${name} --net:"Network 1"="${network}" --diskMode=thin --powerOn --X:waitForIp --X:injectOvfEnv --X:enableHiddenProperties --prop:vami.domain.Harbor=mgmt.local --prop:vami.searchpath.Harbor=mgmt.local --prop:vami.DNS.Harbor=8.8.8.8 --prop:vm.vmname=Harbor harbor_0.4.5_beta.ovf 'vi://${user}:${password}@${host}'
    ${out}=  Split To Lines  ${out}
