...
Disable Ops User And Install VIC To Test Server
    ...
    ${rand_vol_folder}=  Generate Random String  length=6  chars=[LETTERS]
    Install VIC with version to Test Server  ${old_version}  additional-args=--cpu-reservation 1 --cpu-shares normal --memory-reservation 1 --memory-shares normal --endpoint-cpu 1 --endpoint-memory 2048 --base-image-size 8GB --volume-store %{TEST_DATASTORE}/VCH-${rand_vol_folder}-VOL:volumes_${old_version} --bridge-network-range 172.16.0.0/12 --container-network-firewall vm-network:published --certificate-key-size 2048
