...
Simple docker volume inspect
    ...
    ${rc}  ${output}=  Run And Return Rc And Output  docker ${params} volume inspect test
    Should Be Equal As Integers  ${rc}  0
...
Docker volume inspect invalid object
    ${rc}  ${output}=  Run And Return Rc And Output  docker ${params} volume inspect fakeVolume
