...
Docker run --rm concurrent
    ...
    \   ${pid}=  Start Process  docker %{VCH-PARAMS} run -d --rm --name rm-concurrent-${idx}-LOL --cpuset-cpus 1 --memory 1GB ubuntu /bin/sh -c 'for i in `seq 0 75`; do echo $i; sleep 2; done'  shell=True
    ...
    Wait Until Keyword Succeeds  4 min  10 sec  Verify container is removed  rm-concurrent-0-LOL
