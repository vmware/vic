...
Connect containers to multiple networks overlapping
    ${status}=  Get State Of Github Issue  2498
    Run Keyword If  '${status}' == 'closed'  Fail  Test 1-17-Docker-Network-Connect.robot needs to be updated now that Issue #2498 has been resolved
    Log  Issue \#2498 is blocking implementation  WARN
