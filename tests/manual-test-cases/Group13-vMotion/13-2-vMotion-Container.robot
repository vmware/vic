...
Custom vMotion A VM
    ...
    Run Keyword If  ${status}  Run  govc vm.migrate -host cls/${esx1_ip} -pool cls/Resources ${vm}
    Run Keyword Unless  ${status}  Run  govc vm.migrate -host cls/${esx0_ip} -pool cls/Resources ${vm}
...
Test
   ...
   # Set Suite Variable  @{list}  ${user}-vic-vmotion-13-2.vcva-${VC_VERSION}  ${user}-vic-vmotion-13-2.esx.0  ${user}-vic-vmotion-13-2.esx.1  ${user}-vic-vmotion-13-2.esx.2  ${user}-vic-vmotion-13-2.esx.3  ${user}-vic-vmotion-13-2.nfs.0  ${user}-vic-vmotion-13-2.iscsi.0
    ...
    Custom vMotion A VM  ${vmName1}
    Custom vMotion A VM  ${vmName2}
    Custom vMotion A VM  ${vmName3}
