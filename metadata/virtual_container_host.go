...
type VirtualContainerHostConfigSpec struct {
	...
	ExecutorConfig `vic:"0.1" recurse:"depth=0"`
	...
	VolumeLocations []url.URL `vic:"0.1" recurse:"depth=0"`
	// Permitted datastore URLs for container storage for this virtual container host
	ContainerStores []url.URL `vic:"0.1" recurse:"depth=0"`
	// Resource pools under which all containers will be created
	ComputeResources []*object.ResourcePool `vic:"0.1" recurse:"depth=0"`
	...
	VCHSize Resources `vic:"0.1" recurse:"depth=0"`
	// Appliance capacity
	ApplianceSize Resources `vic:"0.1" recurse:"depth=0"`

	// Port Layer - exec
	// Default containerVM capacity
	ContainerVMSize Resources `vic:"0.1" recurse:"depth=0"`
	...
	RegistryWhitelist []url.URL `vic:"0.1" recurse:"depth=0"`
	// Blacklist of registries
	RegistryBlacklist []url.URL `vic:"0.1" recurse:"depth=0"`
	...
	ImageFiles []string `vic:"0.1" recurse:"depth=0"`
}

type NetworkInfo struct {
	PortGroupName string                  `vic:"0.1" scope:"read-only" key:"portgroup"`
	Mac           string                  `vic:"0.1" scope:"read-only" key:"mac"`
	PortGroup     object.NetworkReference `vic:"0.1" recurse:"depth=0"`
	InventoryPath string                  `vic:"0.1" recurse:"depth=0"`
}
