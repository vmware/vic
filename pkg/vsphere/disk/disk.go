...
const (
	// These should probably resemble `magic(file)`.  See here for details
	// (https://github.com/file/file/blob/master/magic/Magdir/filesystems).
	_                   = iota
	Ext4 FilesystemType = 1 << (1 * iota)
	Ntfs
)

type Filesystem interface {

	// We're leaving out Mkfs since our disks inherit from a "parent".  That
	// is, they are built from an immutable disk image which has a delta
	// applied to it.  In this case, the parent already has the filesystem
	// created via Mkfs.
