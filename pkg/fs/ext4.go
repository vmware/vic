...
func (e *Ext) Mkfs(devPath, label string) error {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		log.Errorf("vmdk storage driver failed to format disk %s: %s", devPath, errors.ErrorStack(err))
...
func (e *Ext) Mount(devPath, targetPath string, options []string) error {
	defer trace.End(trace.Begin(devPath))
...
func (e *Ext) SetLabel(devPath, labelName string) error {
	defer trace.End(trace.Begin(devPath))

	cmd := exec.Command("/sbin/e2label", devPath, labelName)
	if output, err := cmd.CombinedOutput(); err != nil {
		log.Errorf("failed to set label %s: %s", devPath, errors.ErrorStack(err))
