...
func HandshakeClient(ctx context.Context, conn net.Conn, debug bool) error {
	...
	if debug {
		...
		if _, err = conn.Write([]byte{CodeAck}); err != nil {
			return err
		}
...
func HandshakeServer(ctx context.Context, conn net.Conn) error {
	...
	if n, err := readMultiple(conn, syn); n != 2 || err != nil || syn[0] != CodeSync {
		var msg string
		if err != nil {
			msg = fmt.Sprintf("server: failed to read expected SYN: n=%d, err=%s", n, err)
		} else if syn[0] != CodeSync {
			msg = fmt.Sprintf("server: did not receive SYN (read %v bytes): %#x != %#x", n, CodeSync, syn[0])
			conn.Write([]byte{CodeNak})
		} else {
	...
	if (buf[0] != CodeAck && buf[0] != CodeDebugAck) || bytes.Compare(ack[1:], buf[1:]) != 0 {
