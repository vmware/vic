...
import com.vmware.vsphere.client.automation.vm.vgpu.step.VerifyVgpuProfilesDisabledForPoweredOnVmStep;
...
/*Test class to verify vGPU VM>Edit Settings>vGPU profiles disabled when vm powered on
 * Pre-requisite steps:
 ...
 * 5. Power on VM > verify vGPU profiles combo box list is disabled
 */

public class VerifyGpuProfileDisabledOnPoweredOnVmTest extends NGCTestWorkflow {
   ...
   public void initSpec(WorkflowSpec testSpec, TestBedBridge testbedBridge) {
      ...
      // Spec for the vm that will be edited (init configuration)
      ...
      // Spec for the vm that will be edited (target configuration)
   ...
   public void composeTestSteps(
      ...
      flow.appendStep("Verify vGPU profiles combo box for Shared Pci Device is disabled when VM powered on",
            new VerifyVgpuProfilesDisabledForPoweredOnVmStep());
   ...
   @Override
   @Test(description = "Verify that vGPU settings are disabled for edit on Powered On VM.")
   @TestID(id = "616776")
