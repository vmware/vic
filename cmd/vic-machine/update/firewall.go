...
func (i *UpdateFw) Run(clic *cli.Context) (err error) {
	...
	if i.enableFw {
		...
		op.Warnf("\tThe ruleset %q (vSphere 6.7+) or %q (vSphere 6.5) will be enabled", management.RulesetID, management.VSPCRulesetID)
		op.Warnf("\tThis allows outbound 2377/tcp (%q) or all outbound tcp (%q) traffic from the target", management.RulesetID, management.VSPCRulesetID)
	...
	if i.disableFw {
		...
		op.Warn("### WARNING ###")
		op.Warn("\tThis command modifies the host firewall on the target machine or cluster")
		op.Warnf("\tThe ruleset %q (vSphere 6.7+) or %q (vSphere 6.5) will be disabled", management.RulesetID, management.VSPCRulesetID)
