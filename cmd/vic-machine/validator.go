...
func (v *Validator) Validate(input *Data) (*metadata.VirtualContainerHostConfigSpec, error) {
	...
	vchConfig.VCHSize.CPU.Limit = input.numCPUs
	vchConfig.VCHSize.Memory.Limit = input.memoryMB
...
func (v *Validator) getNetworkPath(net object.NetworkReference) (string, string, error) {
	...
	case *object.DistributedVirtualPortgroup:
		return net.(*object.DistributedVirtualPortgroup).InventoryPath, net.(*object.DistributedVirtualPortgroup).Name(), nil
	case *object.Network:
		return net.(*object.Network).InventoryPath, net.(*object.Network).Name(), nil
	default:
...
func (v *Validator) setNetworks(vchConfig *metadata.VirtualContainerHostConfigSpec) error {
	...
	if err != nil {
		err = errors.Errorf("Failed to get bridge network name: %s", err)
		return err
	}
	...
	path, name, err = v.getNetworkPath(network)
	if err != nil {
		err = errors.Errorf("Failed to get client network name: %s", err)
		return err
	}
	...
	if v.ManagementNetworkName != "" {
		...
		vchConfig.Networks["management"] = &metadata.NetworkInfo{
			PortGroup:     network,
			PortGroupName: v.ManagementNetworkName,
			InventoryPath: v.ManagementNetworkPath,
		}
	}
	return nil
}
