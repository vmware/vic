...
if [ -z "$1" -o -z "$GOVC_URL" -o -z "$vchName" -o -z "$keyf" -o -z "$certf" ]; then
     usage
fi
...
govc vm.change -vm.uuid="${uuid}" -e guestinfo.vch/files="/var/tmp/images/ /var/log/vic/ /home/docker-engine-server/cert.pem /home/docker-engine-server/key.pem"
...
govc vm.change -vm.uuid="${uuid}" -e guestinfo.vch/sbin/docker-engine-server="--host=0.0.0.0 --port=2375 --tls-host=0.0.0.0 --tls-port=2376 --tls-certificate=/home/docker-engine-server/cert.pem --tls-key=/home/docker-engine-server/key.pem"
