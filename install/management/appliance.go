...
func (d *Dispatcher) createApplianceSpec(conf *metadata.VirtualContainerHostConfigSpec) (*types.VirtualMachineConfigSpec, error) {
	...
	spec := &types.VirtualMachineConfigSpec{
		...
		NumCPUs:  int32(conf.VCHSize.CPU.Limit),
		MemoryMB: conf.VCHSize.Memory.Limit,
...
func (d *Dispatcher) setMacToGuestInfo(conf *metadata.VirtualContainerHostConfigSpec) error {
	...
	for key, value := range m {
		...
		for nicName, netInfo := range conf.Networks {
			if netInfo.PortGroupName == key {
				spec.ExtraConfig = append(spec.ExtraConfig, &types.OptionValue{Key: fmt.Sprintf("guestinfo.vch/networks/%s/portgroup", nicName), Value: key})
				spec.ExtraConfig = append(spec.ExtraConfig, &types.OptionValue{Key: fmt.Sprintf("guestinfo.vch/networks/%s/mac", nicName), Value: value})
