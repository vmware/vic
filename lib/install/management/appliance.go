...
func (d *Dispatcher) deleteVM(vm *vm.VirtualMachine, force bool) error {
	...
	if err != nil || power != types.VirtualMachinePowerStatePoweredOff {
		...
		if _, err = tasks.WaitForResult(d.ctx, d.session, vm, func(ctx context.Context) (tasks.Task, error) {
	...
	_, err = tasks.WaitForResult(d.ctx, d.session, vm, func(ctx context.Context) (tasks.Task, error) {
...
func (d *Dispatcher) createAppliance(conf *config.VirtualContainerHostConfigSpec, settings *data.InstallerData) error {
	...
	if d.isVC && d.vchVapp != nil {
		info, err = tasks.WaitForResult(d.ctx, d.session, d.vchVapp, func(ctx context.Context) (tasks.Task, error) {
	...
	// reconfig
	info, err = tasks.WaitForResult(d.ctx, d.session, vm2, func(ctx context.Context) (tasks.Task, error) {
