...
func (d *Dispatcher) deleteSnapshot(id types.ManagedObjectReference, snapshotName string, applianceName string) error {
	...
	if _, err := tasks.WaitForResult(d.ctx, d.session, d.appliance, func(ctx context.Context) (tasks.Task, error) {
...
func (d *Dispatcher) tryCreateSnapshot(name, desc string) (*types.ManagedObjectReference, error) {
	...
	taskInfo, err := tasks.WaitForResult(d.ctx, d.session, d.appliance, func(ctx context.Context) (tasks.Task, error) {
...
func (d *Dispatcher) reconfigVCH(conf *config.VirtualContainerHostConfigSpec, isoFile string) error {
	...
	info, err := tasks.WaitForResult(d.ctx, d.session, d.appliance, func(ctx context.Context) (tasks.Task, error) {
