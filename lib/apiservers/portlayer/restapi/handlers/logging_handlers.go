...
func (i *LoggingHandlersImpl) JoinHandler(params logging.LoggingJoinParams) middleware.Responder {
	h, ok := params.Config.Handle.(string)
	if !ok {
		return logging.NewLoggingJoinInternalServerError().WithPayload(
			&models.Error{Message: "Handle type assertion failed"},
		)
	}
	defer trace.End(trace.Begin(h))

	handle := exec.GetHandle(h)
	if handle == nil {
	...
	res := &models.LoggingJoinResponse{
		Handle: handleprime.(*exec.Handle).String(),
	}
