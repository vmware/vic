...
func (h *vchCreate) buildCreate(op trace.Operation, d *data.Data, finder client.Finder, vch *models.VCH) (*create.Create, error) {
	...
	if vch != nil {
		...
		err := decode.ProcessCompute(op, d, vch, finder)
		if err != nil {
			return nil, err
		}

		err = decode.ProcessNetworks(op, d, vch, finder)
		if err != nil {
			return nil, err
		}

		err = decode.ProcessStorage(op, d, vch, finder)
		if err != nil {
			return nil, err
		}

		// TODO [AngieCris]: I don't like this at all
		...
		err = decode.ProcessCertificates(op, d, vch, networks)
		if err != nil {
			return nil, err
		}

		err = decode.ProcessEndpoint(op, d, vch)
		if err != nil {
			return nil, err
		}

		err = decode.ProcessRegistry(op, d, vch)
		if err != nil {
			return nil, err
		}

		err = decode.ProcessSyslogs(op, d, vch)
		if err != nil {
			return nil, err
		}

		err = decode.ProcessContainer(op, d, vch)
		if err != nil {
			return nil, err
		}
